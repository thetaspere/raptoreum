...
*Setup instructions for a Gitian build of Theta Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Theta
...
- [Building Theta Core](#building-theta-core)
...
Clone the git repositories for Theta Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/theta/theta
...
in the Theta Core repository under 'Fetch and create inputs' to install sources which require
...
To build Theta Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Theta Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/theta/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/theta/theta
...
For example:
```bash
URL=https://github.com/crowning-/theta.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit theta=${COMMIT} --url theta=${URL} ../theta/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit theta=${COMMIT} --url theta=${URL} ../theta/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit theta=${COMMIT} --url theta=${URL} ../theta/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the theta git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../theta/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/theta
SIGPATH=/some/root/path/theta-detached-sigs

./bin/gbuild --url theta=${BTCPATH},signature=${SIGPATH} ../theta/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/theta-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/theta-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/theta-osx-build.assert
```
...
[theta/gitian.sigs](https://github.com/theta/gitian.sigs/) repository, or if that's not possible to create a pull
request.
